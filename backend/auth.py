...
def get_user_permissions(cursor, username: str) -> List[str]:
    ...
    try:
        # 先确保权限表存在
        ensure_permissions_table_exists(cursor)
...
def ensure_permissions_table_exists(cursor):
    """确保权限表存在（在需要时检查并创建）"""
    try:
        # 检查表是否存在
        cursor.execute("SHOW TABLES LIKE 'user_permissions'")
        if not cursor.fetchone():
            print("⚠️ user_permissions 表不存在，正在创建...")
            # TEXT 类型不能有默认值，去掉 DEFAULT '[]'
            cursor.execute("""
...
def init_permissions_table():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            """)
            print("✅ user_permissions 表已就绪")
            
            # 为所有现有用户初始化权限记录
            cursor.execute("SELECT username, user_type FROM users")
            users = cursor.fetchall()
            print(f"📋 找到 {len(users)} 个用户，正在初始化权限记录...")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if data.permissions is not None and len(data.permissions) > 0:
                print(f"更新用户 {username} 权限: {data.permissions}")
                set_user_permissions(cursor, username, data.permissions)
