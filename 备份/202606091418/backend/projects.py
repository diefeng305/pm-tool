...
def get_all_tasks(
    ...
    """
    获取所有任务（支持按优先级、状态筛选，按创建时间/优先级/截止日期排序）
    用于任务中心全景视图
    """
    connection = get_db_connection()
    try:
...
async def complete_task(
    ...
    支持上传多个反馈文件
    """
    operator = request.headers.get("X-Username", "admin")
    ...
    print(f"完成任务 - task_id: {task_id}, feedback: {feedback}, files count: {len(files)}, operator: {operator}")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            insert_operation_log(cursor, operator, "complete_task", "task", task['title'],
                                f"项目: {task['project_name']}, 里程碑: {task['milestone_name']}, 反馈: {feedback or '无'}")
            
            connection.commit()
            return {"code": "SUCCESS", "message": "任务已完成，等待审核", "files": saved_files, "feedback": feedback}
    except Exception as e:
        connection.rollback()
...
def start_task(task_id: int, request: Request):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            connection.commit()
            return {"code": "SUCCESS", "message": "任务已开始", "data": {"status": "in_progress"}}
    except Exception as e:
        connection.rollback()
...
@router.post("/tasks/{task_id}/check-delay")
def check_and_update_task_delay(task_id: int, request: Request):
    """检查并更新任务延误状态（由前端定时调用或手动触发）"""
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT id, due_date, status FROM tasks WHERE id = %s", (task_id,))
            task = cursor.fetchone()
            if not task:
                return {"code": "ERROR", "detail": "任务不存在"}
            
            # 只有未完成的任务才可能标记延误
            if task['due_date'] and task['status'] not in ['completed']:
                today = datetime.now().date()
                if task['due_date'] < today and task['status'] != 'delayed':
                    cursor.execute("UPDATE tasks SET status = 'delayed' WHERE id = %s", (task_id,))
                    connection.commit()
                    return {"code": "SUCCESS", "message": "任务已标记为延误"}
            
            return {"code": "SUCCESS", "message": "任务状态正常"}
    except Exception as e:
        return {"code": "ERROR", "detail": str(e)}
    finally:
        connection.close()
        ...
        with connection.cursor() as cursor:
            ...
            if action == 'approve':
                ...
                if task['milestone_key'] == 'archive':
                    cursor.execute("""
                        UPDATE projects SET completed_date = CURDATE() WHERE code = %s
                    """, (task['project_code'],))
                    print(f"项目 {task['project_code']} 已完成资料归档，项目完结日期设置为今天")
            else:  # reject
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                WHERE action = 'create_task' AND target_name = %s AND target_type = 'task'
                ORDER BY created_at ASC LIMIT 1
            """, (task['title'],))
            log = cursor.fetchone()
            creator = log['operator'] if log else None
            
            if creator and creator != operator:
                return {"code": "ERROR", "detail": "只有任务发布者才能删除此任务"}
            ...
            if stats['total'] == 0:
                ...
                cursor.execute("""
                    ...
                    WHERE project_code = %s AND milestone_key = %s
                """, (task['project_code'], task['milestone_key']))
            elif stats['total'] > 0 and stats['completed'] == stats['total']:
                cursor.execute("""
                    UPDATE project_milestones 
                    SET status = 'completed'
                    WHERE project_code = %s AND milestone_key = %s
...
def delete_project(code: str, request: Request):
    ...
    try:
        ...
        return {"code": "ERROR", "detail": f"删除失败: {str(e)}"}
    finally:
        connection.close()
...
def get_project_milestones(code: str):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT milestone_key, milestone_name, phase, sort_order
...
def publish_milestone_task(
    code: str, 
    milestone_key: str, 
    task_data: PublishTaskRequest,
    request: Request
):
    operator = request.headers.get("X-Username", "admin")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            insert_operation_log(cursor, operator, "create_task", "task", title, 
                                f"项目: {project['name']}, 里程碑: {milestone['milestone_name']}, 审核人: {', '.join(reviewers) if reviewers else '无'}")
...
def update_task_status(task_id: int, req_data: UpdateTaskStatusRequest, request: Request):
    operator = request.headers.get("X-Username", "admin")
    new_status = req_data.status
    
    if new_status not in ['in_progress', 'completed']:
        return {"code": "ERROR", "detail": "无效的状态"}
    ...
    try:
        with connection.cursor() as cursor:
            ...
            action = "complete_task" if new_status == 'completed' else "start_task"
            insert_operation_log(cursor, operator, action, "task", task['title'],
                                f"项目: {task['project_name']}, 里程碑: {task['milestone_name']}")
            
            connection.commit()
            return {"code": "SUCCESS", "message": "任务状态已更新"}
    except Exception as e:
        connection.rollback()
...
def add_project_requirement(code: str, req_data: AddRequirementRequest, request: Request):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            connection.commit()
            return {"code": "SUCCESS", "message": "需求已添加", "requirement": requirement}
    except Exception as e:
        connection.rollback()
...
async def upload_project_image(
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                UPDATE projects SET image_url = %s WHERE code = %s
            """, (image_url, code))
            connection.commit()
            
            insert_operation_log(cursor, operator, "upload_image", "project", project['name'],
                                f"项目编码: {code}")
...
@router.delete("/{code}/image")
def delete_project_image(code: str, request: Request):
    operator = request.headers.get("X-Username", "admin")
    
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT image_url, name FROM projects WHERE code = %s", (code,))
            project = cursor.fetchone()
            if not project:
                return {"code": "ERROR", "detail": "项目不存在"}
            ...
            if project.get('image_url'):
                ...
                cursor.execute("""
                    UPDATE projects SET image_url = NULL WHERE code = %s
                """, (code,))
                connection.commit()
                
                insert_operation_log(cursor, operator, "delete_image", "project", project['name'],
                                    f"项目编码: {code}")
            
            return {"code": "SUCCESS", "message": "图片已删除"}
    except Exception as e:
        connection.rollback()
...
def toggle_project_status(code: str, request: Request):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                UPDATE projects SET status = %s WHERE code = %s
            """, (new_status, code))
            connection.commit()
