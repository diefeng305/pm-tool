...
def init_db():
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # 修改 tasks 表的 status 字段，确保包含 'delayed' 状态
            try:
                cursor.execute("""
                    ALTER TABLE tasks 
                    MODIFY COLUMN status ENUM('pending', 'in_progress', 'completed', 'delayed') DEFAULT 'pending'
                """)
            except Exception as e:
                print(f"修改 tasks.status 字段时忽略: {str(e)}")
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS project_requirements (
                    ...
                    is_original TINYINT DEFAULT 0 COMMENT '是否为新建时录入的原始需求',
                    added_by VARCHAR(50) NOT NULL COMMENT '添加人',
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS milestone_definitions (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    phase VARCHAR(20) NOT NULL COMMENT '前期/中期/后期',
            ...
            if cursor.fetchone()['cnt'] == 0:
                ...
                for m in milestones:
                    cursor.execute("""
                        INSERT INTO milestone_definitions (phase, milestone_key, milestone_name, sort_order)
                        VALUES (%s, %s, %s, %s)
                    """, m)
            ...
            # 初始化序号表：如果为空，则从现有 projects 中计算最大序号
            ...
            if not cursor.fetchone():
                ...
                cursor.execute("""
                    INSERT INTO users (username, password_hash, name, user_type, position, email, is_active)
                    VALUES ('admin', %s, '核心管理员', 'admin', '系统总监', 'admin@renbin.ren', 1)
                """, (admin_pwd,))
